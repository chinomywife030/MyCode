...
export default function MyOrdersTab({ userId }: MyOrdersTabProps) {
  ...
  useEffect(() => {
    async function fetchOrders() {
      ...
      const { data: orders } = await supabase
        ...
        `)
        .or(`buyer_id.eq.${userId},shopper_id.eq.${userId}`)
        .order('created_at', { ascending: false });

      if (orders) {
        const visibleOrders = orders.filter((o) => {
          if (o.buyer_id === userId) return !o.archived_by_buyer;
          if (o.shopper_id === userId) return !o.archived_by_shopper;
          return true;
        });
        setMyOrders(visibleOrders);
        ordersCache.set(userId, { data: visibleOrders, timestamp: now });
      }
      setLoading(false);
    }
    fetchOrders();
  ...
  const updateOrderStatus = async (orderId: string, status: string, wishId?: string) => {
    try {
      ...
      const updated = myOrders.map((o) => (o.id === orderId ? { ...o, status } : o));
      setMyOrders(updated);
      // 清除 cache，強制下次重新載入
  ...
  const handleArchiveOrder = async (order: any) => {
    ...
    if (error) {
      ...
      const updated = myOrders.filter((o) => o.id !== order.id);
      setMyOrders(updated);
      // 清除 cache，強制下次重新載入
      ordersCache.delete(userId);
    }
